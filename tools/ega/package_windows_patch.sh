#!/usr/bin/env bash
# 打包 Windows **patch 版**：scummvm.exe + DLL + 中文資料 + 啟動 .bat，不含遊戲本體。
#
# 這是要上 GitHub Release 的公開版本，[HARD] 絕不能混入任何遊戲資源或 MT-32 ROM。
# full 完整版是另一支 package_windows_full.sh，產物只留本機 dist-all/。
#
...
# 中文靠 --extrapath 指向包內 cht-data 載入字型，玩家不必複製檔案。
# [HARD] 不帶 --language（AGI 遇非英文語言會進不去）、不預設 mt32（本包無 ROM）。
cat > "$STAGE/宇宙傳奇I EGA-繁中版.bat" <<'BAT'
@echo off
chcp 950 >nul
cd /d "%~dp0"
if exist "%~dp0game\LOGDIR" (
  start "" "%~dp0scummvm.exe" --render-mode=ega --extrapath="%~dp0cht-data" --path="%~dp0game" --auto-detect
) else (
  start "" "%~dp0scummvm.exe" --render-mode=ega --extrapath="%~dp0cht-data"
)
BAT
...
【最簡單】把你的遊戲資料夾複製到本包裡、命名為 game，
          然後雙擊「宇宙傳奇I EGA-繁中版.bat」，會直接開始遊戲。

【一般】  直接雙擊「宇宙傳奇I EGA-繁中版.bat」開啟 ScummVM 啟動器，
