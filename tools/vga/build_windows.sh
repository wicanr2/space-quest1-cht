...
if [ ! -d "$WIN" ]; then
  ...
  echo ">> scummvm-win 已存在，同步 engines/sci 的原始碼（**只同步原始碼**）"
  # [雷 3] 不可以 `cp -a engines/sci/.`：scummvm-src 是本機 Linux 編過的樹，裡面有 ELF 的
  ...
  find "$SRC/engines/sci" -name '*.cpp' -o -name '*.h' | while read -r f; do
    rel="${f#$SRC/}"
