...
每字 = big-endian Big5 碼 + H 列 × ceil(W/8) bytes,MSB 在左;檔尾 0xFFFF):
  <out>/sq1_big5.fnt     16×15,低解析路徑(hi-res 缺字時的後備)
  <out>/sq1_big5_hi.fnt  24×24,hi-res 路徑(640×400 display 直繪)
...
# --- 主流程 ----------------------------------------------------------------
def bake(chars, font, out_path, w, h, ttf_path, ttf_face, label, pad_h=None):
    ...
    # [HARD] 每字資料長度必須與**引擎載入時宣告的高度**一致。
    # Big5Font::loadPrefixedRaw(stream, H) 是靠 H 算每字步幅來切檔的，
    # 檔案每字少一列，第 N 個字就會偏 2N bytes——早期的字還看得懂、後面的字
    # 變成別的字或整個消失，而且不會有任何警告（AGI 軌實測踩過：倚天 16x15 是
    # 15 列，AGI 卻用 16 載入）。pad_h 讓輸出補到指定列數（底部補空白列）。
    rowbytes = (w + 7) // 8
    pad_rows = max(0, (pad_h or h) - h)
    with open(out_path, "wb") as f:
        for code, bmp in glyphs:
            f.write(struct.pack(">H", code))
            f.write(bmp)
            if pad_rows:
                f.write(b"\x00" * (rowbytes * pad_rows))
        f.write(struct.pack(">H", 0xFFFF))

    exp = len(glyphs) * (2 + (pad_h or h) * rowbytes) + 2
...
def main():
    ...
    ap.add_argument("--lo-pad-height", type=int, default=None,
                    help="低解析字型補到幾列（AGI 軌要 16，因為引擎用 loadPrefixedRaw(...,16) 讀）")
    a = ap.parse_args()
    ...
    bake(chars, lo, f"{a.outdir}/{a.prefix}_big5.fnt", 16, 15, a.ttf, a.ttf_face, "低解析", pad_h=a.lo_pad_height)
