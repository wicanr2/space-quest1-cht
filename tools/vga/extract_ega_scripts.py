...
def script_strings(path):
    ...
    for c in body.split(b'\x00'):
        try:
            ...
            continue
        # script 內嵌的顯示字串常黏著前導 bytecode(如 b"d\x01Would you like to skip\n...")。
        # 「含控制碼就整條跳過」會把這些字串連同開場提示框、離開確認框一起丟掉，
        # 而覆蓋率數字看不出來。改成**剝掉最後一個控制碼之前的所有 byte**再過濾。
        last_ctrl = max((i for i, ch in enumerate(s) if ord(ch) < 32 and ch != '\n'), default=-1)
        if last_ctrl >= 0:
            s = s[last_ctrl + 1:]
        # \n 要算成可列印。多行硬換行的字串(開場提示框、片頭 crawl、片尾致謝)是
        # 單一字串內含 0x0A，若不算進來，比例會掉到門檻下而**整條被丟掉**——
        # 而且覆蓋率數字完全看不出來，實機才發現那塊是英文。
        pr = sum(1 for ch in s if 32 <= ord(ch) < 127 or ch == '\n')
