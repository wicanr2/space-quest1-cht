...
有些字不是文字，是**畫**在圖裡的，查表換不掉。VGA 版的兩塊招牌因此另外重畫了中文——阿寇達號走廊的閃爍警示牌（`RED ALERT` → 警報）與尤倫斯荒原的酒吧霓虹招牌（`ROCKET` → 火箭酒吧）。做法一樣不碰遊戲資源：把重畫好的點陣放進 `sq1_cels.dat`，引擎解圖時換上去，交付的檔案裡只有自己畫的像素。其餘還留英文的招牌，以及為什麼留著，寫在[已知未完成的部分](docs/known-issues.md)。

<p align="center">
  <img src="docs/assets/vga-sign-cht.png" width="640" alt="阿寇達號走廊，牆上的警示牌顯示中文「警報」">
</p>
